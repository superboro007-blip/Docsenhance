...
export const PassportStudio: React.FC<PassportStudioProps> = () => {
  ...
  // 1-Click Fast AI Background Subtraction & ICAO White Conversion
  const handleQuickAiBgRemove = async (targetColor: string = '#ffffff') => {
    if (!rawImage || isAiBgRemoving) return;
    setIsAiBgRemoving(true);
    setAiBgStatus(
      targetColor === '#ffffff'
        ? 'Isolating portrait & applying ICAO Pure White...'
        : 'AI neural background subtraction in progress...'
    );

    try {
      const result = await removeAiBackground(rawImage, {
      ...
      if (result.dataUrl) {
        setRawImage(result.dataUrl);
        setSettings((prev) => ({ ...prev, backgroundColor: targetColor }));
        setAiBgStatus('Background successfully replaced!');
        confetti({ particleCount: 35, spread: 50, origin: { y: 0.7 } });
      }
    } catch (err) {
      console.warn('Quick AI background subtraction error:', err);
      setAiBgStatus('Fast studio background adjustment applied');
    } finally {
    ...
    >
      ...
      {isDraggingOver && (
        <div className="fixed inset-0 z-50 bg-slate-950/85 backdrop-blur-md flex flex-col items-center justify-center p-6 border-4 border-dashed border-blue-500 animate-fade-in pointer-events-none">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Drop Portrait Photo Here</h2>
          <p className="text-sm text-blue-300 max-w-md text-center">
            Release to instantly load and open the ICAO / Biometric face crop guide.
          </p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-5 space-y-5">
          ...
          <div className="glass-card rounded-2xl p-5 border border-white/10 shadow-lg space-y-4">
            ...
            {/* Drag and drop upload banner inside card */}
            <div
              onClick={() => fileInputRef.current?.click()}
              className="border border-dashed border-blue-500/30 hover:border-blue-400/60 bg-blue-500/5 hover:bg-blue-500/10 rounded-xl p-2.5 text-center cursor-pointer transition-all flex items-center justify-center gap-2"
            >
              <Upload className="w-4 h-4 text-blue-400 shrink-0" />
              <span className="text-xs font-medium text-blue-300">
                Drag & Drop portrait image here, or click to browse
              </span>
            </div>
              ...
              <div className="flex-1 space-y-2">
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 px-3.5 py-2 rounded-xl bg-gradient-to-r from-purple-600/30 to-indigo-600/30 hover:from-purple-600/50 hover:to-indigo-600/50 text-purple-200 text-xs font-bold border border-purple-500/40 shadow-sm transition-all"
                  >
                    <Sparkles className="w-4 h-4 text-purple-400 animate-pulse" />
                    AI Background Studio (Subtract & Replace)
                  </button>
                  <button
                    onClick={() => handleQuickAiBgRemove('#ffffff')}
                    disabled={isAiBgRemoving}
                    className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/10 hover:bg-white/20 text-slate-200 text-xs font-semibold border border-white/15 transition-all disabled:opacity-50"
                  >
                    {isAiBgRemoving ? (
                    ...
                    {isAiBgRemoving ? 'AI Subtraction in Progress...' : '1-Click ICAO Pure White (AI)'}
                  </button>
                  <button
          ...
          <div className="glass-card rounded-2xl p-5 border border-white/10 shadow-lg space-y-4">
            ...
            {activeTab === 'adjustments' && (
              <div className="space-y-4">
                {/* AI Background Subtraction Section */}
                <div className="p-3.5 bg-purple-950/40 border border-purple-500/30 rounded-xl space-y-2.5">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-purple-200 flex items-center gap-1.5">
                      <Sparkles className="w-3.5 h-3.5 text-purple-400" />
                      AI Neural Background Subtraction
                    </span>
                  ...
                  <p className="text-[11px] text-purple-300/80 leading-tight">
                    Uses neural deep segmentation to extract the subject and composite clean passport backgrounds.
                  </p>
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-1">
                    <button
                      onClick={() => handleQuickAiBgRemove('#ffffff')}
                      disabled={isAiBgRemoving}
                      className="px-2.5 py-2 rounded-lg bg-white/10 hover:bg-white/20 text-white text-xs font-bold border border-white/20 transition-all flex items-center justify-center gap-1.5 disabled:opacity-50"
                    >
                      <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" />
                      Pure White (#FFF)
                    </button>
                    <button
                      onClick={() => handleQuickAiBgRemove('#dbeafe')}
                      disabled={isAiBgRemoving}
                      className="px-2.5 py-2 rounded-lg bg-blue-500/20 hover:bg-blue-500/30 text-blue-200 text-xs font-bold border border-blue-500/30 transition-all flex items-center justify-center gap-1.5 disabled:opacity-50"
                    >
                      <span className="w-2.5 h-2.5 rounded-full bg-[#dbeafe] border border-blue-400" />
                      Light Blue
                    </button>
                  </div>
                  ...
                  <button
                    onClick={() => setIsBgRemovalOpen(true)}
                    className="w-full py-2 px-3 rounded-lg bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs font-bold shadow-md shadow-purple-600/20 transition-all flex items-center justify-center gap-1.5"
                  >
                    <Wand2 className="w-3.5 h-3.5" />
                    Open AI Background Studio
                  </button>
                </div>
